# Agent Guidelines for Foundatio.AzureServiceBus
...
```bash
# Build
dotnet build Foundatio.AzureServiceBus.slnx

# Test
dotnet test Foundatio.AzureServiceBus.slnx

# Format code
dotnet format Foundatio.AzureServiceBus.slnx
```
...
```text
src
└── Foundatio.AzureServiceBus    # Azure Service Bus implementations
    ├── Queues                   # IQueue<T> implementations for Azure Service Bus
    └── Extensions               # Extension methods for DI
tests
└── Foundatio.AzureServiceBus.Tests  # Unit and integration tests
```
...
1. **Builds successfully**: `dotnet build Foundatio.AzureServiceBus.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.AzureServiceBus.slnx` shows no failures
...
- Mirror the main code structure (e.g., `Queues/` tests for `src/Foundatio.AzureServiceBus/Queues/`)
...
# Specific test file
dotnet test --filter "FullyQualifiedName~AzureServiceBusQueueTests"
