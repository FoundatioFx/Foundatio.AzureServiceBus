# Copilot AI Coding Agent Instructions for Foundatio.AzureServiceBus
...
- `src/Foundatio.AzureServiceBus/` — Main library code for AzureServiceBus integration.
- `tests/Foundatio.AzureServiceBus.Tests/` — Unit and integration tests for AzureServiceBus features.
- `build/` — Shared build props, strong naming key, and assets.
- `Foundatio.AzureServiceBus.slnx` — Solution file for development.

## Developer Workflows
- **Build:** Use the VS Code task `build` or run `dotnet build` at the repo root.
- **Test:** Use the VS Code task `test` or run `dotnet test tests/Foundatio.AzureServiceBus.Tests`.
